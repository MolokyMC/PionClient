package genshin.pion.Pion.gui.UI;
...
public class ClickGUI extends GuiScreen {
    ...
    private List<genshin.pion.Pion.gui.UI.Panel> panels = new ArrayList<>();

    public ClickGUI() {
        int x = 20;

        for(final ModuleType moduleCategory : ModuleType.values()) {
            final genshin.pion.Pion.gui.UI.Panel panel = new genshin.pion.Pion.gui.UI.Panel(moduleCategory.getName(), x, 50, 100);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        fontRendererObj.drawStringWithShadow("Pion is a client, if you pay any money for it mean you got deceived.", 5, this.height - 10, new Color(255, 0, 0).hashCode());
        for(final genshin.pion.Pion.gui.UI.Panel panel : panels) {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        if(mouseButton == 0) {
            for(int index = panels.size() - 1; index >= 0; index--) {
                final genshin.pion.Pion.gui.UI.Panel panel = panels.get(index);
